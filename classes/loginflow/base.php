...
class base {
    ...
    public function __construct() {
        $default = [
            'opname' => get_string('pluginname', 'auth_classlink')
        ];
        $storedconfig = (array)get_config('auth_classlink');
        $forcedconfig = [
    ...
    public function get_userinfo($username) {
        ...
        $idtoken = \auth_classlink\jwt::instance_from_encoded($tokenrec->idtoken);
        /* echo '<pre>',print_r($idtoken,1),'</pre>';die(); */
     ...
     *
     * @param auth_classlinkhttpclientinterface $httpclient [description]
     */
    public function set_httpclient(\auth_classlink\httpclientinterface $httpclient) {
        $this->httpclient = $httpclient;
    }

    /**
     * Handle classlink disconnection from Moodle account.
     *
     * @param bool $justremovetokens If true, just remove the stored classlink tokens for the user, otherwise revert login methods.
    ...
    public function disconnect($justremovetokens = false, $donotremovetokens = false, \moodle_url $redirect = null,
        ...
        global $USER, $DB, $CFG;
        if ($redirect === null) {
            $redirect = new \moodle_url('/auth/classlink/ucp.php');
        }
        if ($selfurl === null) {
            $selfurl = new \moodle_url('/auth/classlink/ucp.php', ['action' => 'disconnectlogin']);
        }
        ...
        if ($justremovetokens === true) {
            // Delete token data.
            $DB->delete_records('auth_classlink_token', ['userid' => $userrec->id]);
            $eventdata = ['objectid' => $userrec->id, 'userid' => $userrec->id];
            $event = \auth_classlink\event\user_disconnected::create($eventdata);
            $event->trigger();
            redirect($redirect);
        } else {
            global $OUTPUT, $PAGE;
            ...
            $ucptitle = get_string('ucp_disconnect_title', 'auth_classlink', $this->config->opname);
            $PAGE->navbar->add($ucptitle, $PAGE->url);
            $PAGE->set_title($ucptitle);

            // Check if we have recorded the user's previous login method.
            $prevmethodrec = $DB->get_record('auth_classlink_prevlogin', ['userid' => $userrec->id]);
            ...
            if (empty($prevauthmethod) && is_enabled_auth('manual') !== true) {
                throw new \moodle_exception('errornodisconnectionauthmethod', 'auth_classlink');
            }

            // Check to see if the user has a username created by classlink, or a self-created username.
            // classlink-created usernames are usually very verbose, so we'll allow them to choose a sensible one.
            // Otherwise, keep their existing username.
            $classlinktoken = $DB->get_record('auth_classlink_token', ['userid' => $userrec->id]);
            $ccun = (isset($classlinktoken->classlinkuniqid) && strtolower($classlinktoken->classlinkuniqid) === $userrec->username) ? true : false;
            $customdata = [
            ...
            $mform = new \auth_classlink\form\disconnect($selfurl, $customdata);
            ...
            if ($mform->is_cancelled()) {
                ...
                if (empty($fromform->newmethod) || ($fromform->newmethod !== $prevauthmethod && $fromform->newmethod !== 'manual')) {
                    throw new \moodle_exception('errorauthdisconnectinvalidmethod', 'auth_classlink');
                }
                ...
                if ($fromform->newmethod === 'manual') {
                    if (empty($fromform->password)) {
                        throw new \moodle_exception('errorauthdisconnectemptypassword', 'auth_classlink');
                    }
                    if ($customdata['canchooseusername'] === true) {
                        if (empty($fromform->username)) {
                            throw new \moodle_exception('errorauthdisconnectemptyusername', 'auth_classlink');
                        }
                        ...
                        if (strtolower($fromform->username) !== $userrec->username) {
                            ...
                            if ($DB->record_exists('user', $usercheck) === false) {
                                ...
                                throw new \moodle_exception('errorauthdisconnectusernameexists', 'auth_classlink');
                            }
                        }
                    }
    ...
    /**
     * Construct the ClassLink OAuth2 client.
     *
     * @return \auth_classlink\classlinkclient The constructed client.
     */
    protected function get_classlinkclient() {
        global $CFG;
        if (empty($this->httpclient) || !($this->httpclient instanceof \auth_classlink\httpclientinterface)) {
            $this->httpclient = new \auth_classlink\httpclient();
        }
        if (empty($this->config->clientid) || empty($this->config->clientsecret)) {
            throw new \moodle_exception('errorauthnocreds', 'auth_classlink');
        }
        if (empty($this->config->authendpoint) || empty($this->config->tokenendpoint)) {
            throw new \moodle_exception('errorauthnoendpoints', 'auth_classlink');
        }
        ...
        $redirecturi .= '/auth/classlink/';
        $resource = (isset($this->config->classlinkresource)) ? $this->config->classlinkresource : null;

        $client = new \auth_classlink\classlinkclient($this->httpclient);
     ...
     * @return array List of classlinkuniqid and constructed idtoken jwt.
     */
    protected function process_idtoken($idtoken, $orignonce = '') {
        // Decode and verify idtoken.
        $idtoken = \auth_classlink\jwt::instance_from_encoded($idtoken);
        $sub = $idtoken->claim('sub');
        if (empty($sub)) {
            \auth_classlink\utils::debug('Invalid idtoken', 'base::process_idtoken', $idtoken);
        ...
        if (!empty($orignonce) && (empty($receivednonce) || $receivednonce !== $orignonce)) {
            \auth_classlink\utils::debug('Invalid nonce', 'base::process_idtoken', $idtoken);
            throw new \moodle_exception('errorauthinvalididtoken', 'auth_classlink');
        }

        // Use 'oid' if available (Azure-specific), or fall back to standard "sub" claim.
        $classlinkuniqid = $idtoken->claim('oid');
        if (empty($classlinkuniqid)) {
            $classlinkuniqid = $idtoken->claim('sub');
        }
        return [$classlinkuniqid, $idtoken];
    }
     ...
     *
     * @param \auth_classlink\jwt $idtoken The ID token of the user who is trying to log in.
     * @return bool Whether the restriction check passed.
     */
    protected function checkrestrictions(\auth_classlink\jwt $idtoken) {
        $restrictions = (isset($this->config->userrestrictions)) ? trim($this->config->userrestrictions) : '';
        $hasrestrictions = false;
        $userpassed = false;
        if ($restrictions !== '') {
            $restrictions = explode("\n", $restrictions);
            // Match "UPN" (Azure-specific) if available, otherwise match classlink-standard "sub".
            ...
            foreach ($restrictions as $restriction) {
                ...
                if ($restriction !== '') {
                    ...
                    try {
                        ...
                        \auth_classlink\utils::debug('Error running user restrictions.', 'handleauthresponse', $debugdata);
                    }
                    ...
                    if (!empty($contents)) {
                        ...
                        \auth_classlink\utils::debug('Output while running user restrictions.', 'handleauthresponse', $debugdata);
                    }
                }
            }
        }
    ...
    /**
     * Create a token for a user, thus linking a Moodle user to an ClassLink OAuth2 user.
     *
    ...
    protected function createtoken($classlinkuniqid, $username, $authparams, $tokenparams, \auth_classlink\jwt $idtoken, $userid = 0) {
        ...
        $tokenrec->classlinkusername = $classlinkusername;
        $tokenrec->scope = !empty($tokenparams['scope']) ? $tokenparams['scope'] : 'openid profile email';
        $tokenrec->resource = !empty($tokenparams['resource']) ? $tokenparams['resource'] : $this->config->classlinkresource;
    ...
    protected function updatetoken($tokenid, $authparams, $tokenparams) {
        ...
        $tokenrec->refreshtoken = !empty($tokenparams['refresh_token']) ? $tokenparams['refresh_token'] : ''; // TBD?
        $tokenrec->idtoken = $tokenparams['id_token'];
        $DB->update_record('auth_classlink_token', $tokenrec);
    }
}
